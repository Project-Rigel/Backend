...
export const getTimeAvaliableFunction = functions.https.onRequest(async (req, res) => {

    //TODO check auth with onCall function
    const dto: GetAvailableTimesDto = plainToClass(GetAvailableTimesDto, req.body);
    ...
    if (!doc.exists) {
        await doc.ref.set({jorge: "Comeme los pelos", id: `${formattedDateDate}-${dto.agendaId}`});
